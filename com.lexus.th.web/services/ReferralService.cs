...
SELECT [id]
  ...
  FROM [dbo].[booking]
WHERE		deleted_flag IS NULL and Type = 2
...
INSERT INTO [dbo].[booking]   ([title_en],[title_th],[desc_en],[desc_th],[condition_en],[condition_th],[reg_period_en]
                                ...
                                ,[thankyou_message_th],[code_message_en],[code_message_th],[is_active],[created_date],[created_user],[type],[is_required_plate_no],[preferred_model_ids],[max_used],[referral_condition])
		   VALUES
           (
            @title_en
           ,@title_th              
...
INSERT INTO [upload_Image]
     ...
     VALUES
           ...
           ,@created_user)", img.Type, img.FileName, img.FilePath, img.OriginalFileName);
                    ...
                    db.ParamterList.Add(new SqlParameter("@max_used", booking.MaxUsed));
                    db.ParamterList.Add(new SqlParameter("@referral_condition", booking.FollowingBy));
...
INSERT INTO [upload_Image]
     ...
     VALUES
           ...
           ,@updated_user)", booking.Id, img.Type, img.FileName, img.FilePath, img.OriginalFileName);
                    ...
                    db.ParamterList.Add(new SqlParameter("@code_message_en", booking.CodeMessageEN)); 
                    db.ParamterList.Add(new SqlParameter("@code_message_th", booking.CodeMessageTH));
                    if (booking.FollowingBy == "ByCar")
                    {
                        db.ParamterList.Add(new SqlParameter("@max_used", DBNull.Value));
                    }
                    else
                    {
                        db.ParamterList.Add(new SqlParameter("@max_used", booking.MaxUsed));
...
WHERE [Parent_Id]=@ParrentId AND [Type]=@Type AND [Page]='REFERRAL'
ORDER BY ID";
                using (DBAccess db = new DBAccess(conn, DBAccess.ConnectionType.SQLDB))
                {
                    cmd = string.Format(cmd,
