...
{
    public partial class PageAuthDealer : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    else
                    {
                        ...
                        ViewState[holiday] = new DataTable();
        ...
        protected void btnAddModal_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                lbDuplicate.Text = "";
                tbDate.Text = "";
                tbCallCenterMobile.Text = "555";
                tbCallCenterEmail.Text = "555";
                dlPinMapIcon.SelectedIndex = -1;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DealerService srv = new DealerService();
                    if (lbType.Text == "Add")
                    {
                        srv.AddDealer(geo_id, dealer_name, dealer_code, branch_name, branch_code, address,
                            office_hours, office_hours2, mobile, active, user, lat, lng, dealer_name_th,
                            branch_name_th, address_th, office_hours_th, office_hours2_th, dealer_duration_min, dealer_duration_max, pin_map_icon_id, is_pickup_service, appointMent, testDrive, hollidays, booking, province_id);
                        BindGridData();
                        ClientPopup(ModalType.Success, "Completed");
                    }
                    else if (lbType.Text == "Edit")
                    {
                        srv.UpdateDealer(geo_id, dealer_name, dealer_code, branch_name, branch_code, address,
                            office_hours, office_hours2, mobile, active, user, lbId.Text, lat, lng, dealer_name_th,
                            branch_name_th, address_th, office_hours_th, office_hours2_th, dealer_duration_min, dealer_duration_max, pin_map_icon_id, is_pickup_service, appointMent, testDrive, hollidays, booking, province_id);
                        BindGridData();
                        ClientPopup(ModalType.Success, "Completed");
                    }
            ...
            try
            {
                ...
                if (gvDealer.DataKeys != null)
                {
                    ...
                    if (row != null)
                    {
                        ...
                        //dlProvince.SelectedValue = row["province_id"] != null ? row["province_id"].ToString() : "0";
                        BindProvince(Convert.ToInt32(dlGeo2.SelectedValue));
                        ...
                        //get data DealerWorking ServiceAppointment and Test Drive 
        ...
        private void BindGridData()
        {
            try
            {
                ...
                gvDealer.DataSource = srv.GetDealers(dlGeo.SelectedValue,dlIsPickServiceCriteria.SelectedValue);
                gvDealer.DataBind();
            }
            catch (Exception ex)
            {
        ...
        private void BindGeo()
        {
            ...
            {
                DealerService srv = new DealerService();
                dlGeo.DataSource = srv.GetGeoSearchAuthDealer();
                dlGeo.DataBind();
        ...
        private void BindProvince(int geo_id)
        {
            try
            {
                //dlProvince.Items.Clear();
                //ProvinceService srv = new ProvinceService();
                //List<ProvinceModel> datas = srv.GetProvinceByRegion("EN", geo_id);
                ...
                ProvinceService srv = new ProvinceService();
                dlProvince.DataSource = srv.GetProvinceByRegion("EN", geo_id);
                dlProvince.DataBind();
        ...
        protected void dlGeo2_SelectedIndexChanged(object sender, EventArgs e)
        {
            BindProvince(Convert.ToInt32(dlGeo2.SelectedValue));
            upModalAdd.Update();
        }
