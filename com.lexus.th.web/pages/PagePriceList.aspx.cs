...
{
    public partial class PagePriceList : System.Web.UI.Page
    {
        private enum ModalType { Success, Error, Warning }
        private enum ImageType { Img1, Img2 }
            ...
            try
            {
                if (!Page.IsPostBack)
                {
                    ...
                    else
                    {
                        ...
                        ViewState["ContentPricePath"] = System.Web.Configuration.WebConfigurationManager.AppSettings["ContentPricePathConfig"];
                        
                        ConfigService srv = new ConfigService();
                        DataRow row = srv.GetConfigPriceList();
                        if (row != null)
                        {
                            tbImg1.Text = ImageService.Split1(row["pricelist_banner"].ToString());
                            tbURLAddress.Text = row["pricelist_url_address"].ToString();
                        }
                    }
                }
            }
        ...
        private void UploadContent(ImageType type)
        {
            try
            {
                string imgDir = ViewState["ContentPricePath"] + "\\";

                FileUpload fileUpload = upImg2;
                TextBox tbImage = tbImg2;
                Label lbStatus = lbImg2;
                ...
                if (fileUpload.HasFile)
                {
                    ...
                    tbURLAddress.Text = WebConfigurationManager.AppSettings["ContentPriceURLConfig"] + fileUpload.FileName;
                }
                else
                {
                    throw new Exception("No file selected!");
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (lbImg2.Text != "")
                {
                    DeleteContent(ImageType.Img2);
                }

                string pricelist_banner = (tbImg1.Text.Length > 0) ? ImageService.Split2(ViewState["ImagePath"].ToString() + "\\" + tbImg1.Text) : "";
                string pricelist_url_address = tbURLAddress.Text;
                string user = Session["User"].ToString();

                

                ConfigService srv = new ConfigService();
                srv.UpdateConfigPriceList(pricelist_banner, pricelist_url_address, user);
